...
DBLP_PERSON_URL = DBLP_BASE_URL + 'pers/xx/k/{}'
...
class Author(LazyAPIData):
    ...
    def load_data(self):
        ...
        xml = resp.content
        self.xml = xml
        root = make_dict_from_tree(etree.fromstring(xml))
        data = {
            'name': root['dblpperson']['person']['author'],
            'affiliation': root['dblpperson']['person']['note'],
            'publications': root['dblpperson']['r'],
            'homepages': root['dblpperson']['person']['url']
        }
        if isinstance(data['affiliation'], list):
            if data['name'] in data['affiliation']:
                data['affiliation'].remove(name)
            data['affiliation'] = max(data['affiliation'], key=lambda x:len(x))
...
def search_author(author_str):
    search_string = DBLP_AUTHOR_SEARCH_URL + '/api'
    resp = requests.get(search_string,
                        params={'q': author_str, 'format': 'json'})
    print(resp.url)
    authors = json.loads(resp.text)

    results = []
    executor = concurrent.futures.thread.ThreadPoolExecutor(max_workers=500)
    futures = []
    for hit in authors['result']['hits']['hit']:
        url = hit['info']['url']
        if "http://dblp.org/pid/" not in url:
            continue
...
def search_publication(publ_str, years, venue=None):
    ...
    r_executor = concurrent.futures.thread.ThreadPoolExecutor(max_workers=500)
    futures, respones = [], []
    ...
    while futures:
        ...
        if first + sent < total:
            ...
            url = get_request.url[:get_request.url.find("&f=")] + "&f=" + str(current_first)
            futures.append(r_executor.submit(requests.get,
                                             url))
    return respones
