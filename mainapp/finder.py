...
class ScholarlyAccess:
    def __init__(self):
        pass

    def _msearch_keywords(self, keywords):
        authors = []
    ...
    def find(self, criteria):
        ...
        if 'affiliation' in criteria and criteria['affiliation']:
        ...
        for author in authors:
            author.fill()
            entry = {
                'name': flatten([author.name.replace('autor', '').strip(':"')]),
                'affiliation': flatten([author.affiliation]),
                'interests': flatten([author.interests]),
                'image': ['https://scholar.google.pl' + author.url_picture]
            }
            print(author._filled)
            futures = [executor.submit(x.fill) for x in author.publications]
            [x.result() for x in futures]
            entry['publications'] = [x.bib for x in author.publications]
            new_pubs = []
            for pub in entry['publications']:
                new_pub = {
                    ...
                    'year': [pub.get('year', '')],
                    'link': [pub.get('url', '')],
                    'author': flatten([pub.get('author', '').split('and')])
                }
                other = ""
                if 'publisher' in pub:
                    other += "Publisher: {} ".format(pub.get('publisher'))
                if 'journal' in pub:
                    other += "Journal: {} ".format(pub.get('journal'))
                new_pub['other'] = [other]
                new_pubs.append(new_pub)
            entry['publications'] = new_pubs
    ...
    def refine_publication(self, author):
        ...
        for pub in pubs:
            ...
            if pub_type in ['inproceedings', 'article', 'book']:
                ...
                pub['other'] = ''
                if pub_type == 'book':
                    pub['author'] = pub.get('editor', '') or pub['author']
                    pub['title'] = pub.get('booktitle', '') or pub['title']
    ...
    def find(self, criteria):
        ...
        for author in authors:
            ...
            if not isinstance(author['name'], str):
                author['name'] = unidecode(author['name'][0])
...
class ORCiD:
    ...
    def process_author(self, author):
        ...
        if new_entry['biography']:
            new_entry['biography'] = new_entry['biography']['value']
    ...
    def process_publications(self, pubs):
        ...
        for pub in pubs:
            ...
            if 'journal-title' in pub._original_dict:
                new_pub['journal'] = pub._original_dict['journal-title']
